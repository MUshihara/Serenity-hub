...
-- Dedicated route using the maintained Access V2 implementation.
-- V2 hard-patches the LootLabs provider before the key UI is compiled.
...
local url=TEMPLATE.."?cut_grass_access_v2="..tostring(os.time())..tostring(math.random(100000,999999))
...
local function replaceExact(old,new,label)
    ...
    if not p then
        error("[SERENITY HUB] Cut Grass access template mismatch: "..tostring(label),0)
    end
...
replaceExact(
    ...
    '[SERENITY HUB] This route is only for +1 Cut Grass Adventure.',
    "route message"
)
replaceExact(
    '{name="Game",value="+1 Superhero Evolution",inline=true}',
    '{name="Game",value="+1 Cut Grass Adventure",inline=true}',
    "webhook game"
)
replaceExact(
    '[SERENITY HUB] +1 Superhero Evolution payload is unavailable.',
    '[SERENITY HUB] +1 Cut Grass Adventure payload is unavailable.',
    "payload message"
)
replaceExact(
    '"@Serenity/Games/Plus1SuperheroEvolution"',
    '"@Serenity/Games/Plus1CutGrassAdventure"',
    "payload chunk"
)
replaceExact(
    '"@SerenityHub/AccessV2-Superhero-Direct"',
    '"@SerenityHub/AccessV2-CutGrass-Direct"',
    "access chunk"
)

-- Critical provider override. This is deliberately patched into the Access V2
-- template itself so the key-system button cannot fall back to the old URL.
replaceExact(
    'https://lootdest.org/s?a51Wabb2',
    'https://loot-link.com/s?jbyyXFuO',
    "LootLabs provider"
)

local fn,err=loadstring(source,"@SerenityHub/AccessV2-CutGrass-V2")
source=nil
if not fn then
    error("[SERENITY HUB] Cut Grass Access V2 compile failed: "..tostring(err),0)
end
return fn()
