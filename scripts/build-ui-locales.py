...
packs={l:{} for l in ['fil','id','vi','th','es','pt','fr','de','ru']}
for line in (root/'translations.txt').read_text().splitlines():
 parts=line.split('|');assert len(parts)==10,parts[0]
 for lang,text in zip(packs,parts[1:]):
  assert parts[0] not in packs[lang];packs[lang][parts[0]]=text
  if parts[0] in ['Community','Updates']:packs[lang][parts[0].upper()]=text
core=(root/'i18n-core.lua').read_text()
