# Phonk Evolution language preview v1
...
It opens Settings > Appearance. Switch Language there. Automatic (Roblox) uses LocalizationService.RobloxLocaleId. Unsupported locales fall back to English. Manual selection wins over Roblox locale changes. Ten languages: English, Filipino, Indonesian, Vietnamese, Thai, Spanish, Brazilian Portuguese, French, German, Russian. Portuguese regional variants currently use the Brazilian pack; tl/fil map to Filipino.
...
- Choose Automatic, then compare with Roblox's configured language (not your physical location).
...
The Python runner uses liblua5.4.so.0, checks the shared Lua/Luau syntax subset and mocked Roblox behavior. It is not an in-engine visual test. Tests cover 117 entries per translation pack, all Phonk control titles, locale aliases, dynamic strings, saved settings, unchanged canonical values, draft preservation, popup cleanup, viewport changes and public-runtime isolation. Visual sizing/font rendering still requires PC/mobile testing.
